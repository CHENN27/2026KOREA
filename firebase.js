...
async function joinCloudTrip(tripId) {
  ...
  try {
    ...
    if (!data.members.includes(uid)) {
      ...
      // 等待 members 更新確認（使用 onSnapshot 確認而非固定延遲）
      await new Promise((resolve, reject) => {
        const timeout = setTimeout(() => { unsub(); reject(new Error('等待 members 更新逾時')); }, 5000);
        const unsub = tripRef.onSnapshot((snap) => {
          if (snap.exists && snap.data().members.includes(uid)) {
            clearTimeout(timeout);
            unsub();
            resolve();
          }
        }, (err) => { clearTimeout(timeout); unsub(); reject(err); });
      });
    }
...
async function savePlanDoc(tripId, plan) {
  ...
  try {
    // 建立乾淨副本，避免修改原始物件
    const { updatedAt, ...cleanPlan } = plan;
...
async function saveExpenseDoc(tripId, expense) {
  ...
  try {
    // 建立乾淨副本，避免修改原始物件
    const { updatedAt, ...cleanExpense } = expense;
    await db.collection('trips').doc(tripId)
